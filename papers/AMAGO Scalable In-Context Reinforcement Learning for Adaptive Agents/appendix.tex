...
% We explore the upper bound of AMAGO's performance in select environments by either leaving the standard agent running for additional timesteps (``AMAGO-Long''), or training a deeper $5$ layer Transformer for $100$M timesteps (``AMAGO-XL''). We briefly summarize these results as a reference for future work on this benchmark:
% \begin{itemize}
%     \item AMAGO-Long consistently converges to the plotted upper bound on CountRecallHard and AutoencodeHard by $20$M timesteps.
%     \item AMAGO-XL converges to an average return of $.20$ on ConcentrationMedium at approximately $75$M timesteps.
%     \item AMAGO-XL converges to an average return of $.33$ on MineSweeperMedium at approximately $80$M timesteps.
%     \item AMAGO-XL does not improve performance on MineSweeperEasy.
%     \item AMAGO-XL converges to an average return of $.49$ on BattleshipEasy at approximately $35$M timesteps.
% \end{itemize}
...
\input{crafter_single_goal_table}
...
% \section{Addendum: Mamba in AMAGO}
% \label{app:mamba}



% AMAGO's trajectory encoder is responsible for memory and state/environment estimation. Our work focuses on Transformers because their attention mechanism allows for an unlimited hidden state size and turns recall into a read-only retrieval process. RNN-like models have the advantage of constant-time inference but need to learn to manage memory by writing and deleting information at each timestep. However, our code release is compatible with any seq2seq model, which may be convenient for research in hybrid memory architectures. For example, we can replace the Transformer with Mamba \cite{gu2023mamba}. Mamba makes a similar trade-off to RNNs but maintains its memory in an expanded hidden state with the stability of a state space model \cite{gu2021efficiently}. We test ``AMAGO-Mamba'' in a sample of environments where our results suggest Transformers are essential. We default to the hyperparameters used by the Transformer version but experiment with a few changes inspired by the results and discussion in the Mamba paper \cite{gu2023mamba}. Results are shown in Figure \ref{fig:mamba_eval}. Mamba is an exciting alternative to attention, but we are not (yet) able to replicate the Transformer performance in all of the environments. In general, toy long-term memory and meta-RL environments may highlight the ``state space vs. attention'' debate more directly than many supervised learning benchmarks. RL's bootstrapped credit assignment tends to create cycles where memory can only improve when returns increase, but returns will only increase after memory improves. Transformers' automatic retention of the entire sequence breaks the cycle by removing the need to learn proactive memory management. For this reason it seems conceptually difficult to justify choosing an alternative encoder when the adaptation or memory problem requires context lengths short enough to make Transformers practical. However, challenging new benchmarks requiring long-term memory and adaptation over $10,000+$ timesteps could make state space models an efficient default \cite{lu2023structured}.

% \begin{figure}[h!]
%     \centering
%     \includegraphics[width=\textwidth]{figs/mamba_eval.pdf}
%     \caption{\textbf{Mamba as AMAGO's Trajectory Encoder in a Sample of CMDPs.} The dashed green line indicates Transformer performance at the final timestep plotted. We experiment with changes to the learning rate, model depth, and hidden state expansion factor inspired by the Mamba results.}
%     \label{fig:mamba_eval}
% \end{figure}