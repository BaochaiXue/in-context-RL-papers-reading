...
% In addition, we test the limits of in-context RL generalization by finding the maximum degree of suboptimality and discovering a pattern in the ideal policy improvement pace between transitions in the learning data.

% We propose ADε, a new data acquisition approach that enables in-context Reinforcement Learning. While other in-context RL methods impose strict restrictions on the data to ensure that it is gener- ated by RL agents or labeled with actions from an optimal policy, our approach gives a rise to in-context learning from noise distillation by con- structing synthetic learning histories. We demon- strate that it is possible to alleviate the need of optimal generating policies, so that in-context RL is able to outperform the best suboptimal policy in learning dataset by a 2x margin.
% We also present an analysis on a learning pace  
\end{abstract}
...
% \cite{laskin2022context, lee2023supervised, kirsch2023towards, sinii2023context}
% More recent approaches leverage the self-attention mechanism of Transformers \cite{vaswani2017attention} 
...
% this can give rise to applying In-Context RL to the real world tasks

% Such data is not easily accessible; existing datasets with trajectories \cite{fu2020d4rl, robomimic2021} only contain trajectories without sequential improvement between transitions, thus making them unsuitable for training Meta-RL algorithms. Moreover, each problem may require its own learning pace in the data, which poses additional challenge in training of single-task RL algorithms.
...
% Our findings are backed up by a theoretical research of \citet{jeon2024informationtheoretic}, where they show that the misspecification error between Bayesian predictors that trained on optimal and suboptimal algorithms is can effectively be upper bounded by zero.
...
%     We show that it is possible to replace actual learning histories with synthetically generated without hurting the performance of in-context RL algorithm. We illustrate our findings in \autoref{fig:optimal_results}.
%     \item \textbf{As a consequence, supervised pre-training does not require trajectories from RL agents or supь ervision with optimal actions for emergence of In-Context Reinforcement Learning.} 
%     \item It is possible to improve upon suboptimal demonstrators with pure in-context learning after supervised pre-training. 
% \end{itemize}
...
% \vskip 0.2in
% \begin{center}
%     \centerline{\includegraphics[width=0.6\textwidth]{icml/img/main.pdf}}
%     \caption{Data acquisition for in-context RL training. While other in-context RL methods either train thousands single-task RL algorithms to obtain their learning histories (AD) or pretrain on optimal actions (DPT), our approach AD$^\varepsilon$ alleviates these problems by synthetically generating learning histories. The algorithm trained on synthetic data not only does generalize to unseen tasks, but also can outperform the best available policy in data ($\pi^\text{data}$).}
%     \label{fig:main}
% \end{center}
% \vskip -0.2in
% \end{figure*}
...
% \begin{itemize}
%     \item Distilling noise-induced trajectories of a demonstrator gives rise to In-Context RL.
%     \item As a consequence, supervised pre-training does not require trajectories from RL agents or supervision with optimal actions for emergence of In-Context RL
%     \item It is possible to improve upon suboptimal demonstrators 
% \end{itemize}

% \begin{figure*}[h]
% \vskip 0.2in
% \begin{center}
%     \centerline{\includegraphics[width=\textwidth]{icml/img/optimal_dr_k2d_wm.pdf}}
%     \caption{TODO}
...
\vskip 0.2in
\begin{center}
    \centerline{\includegraphics[width=\textwidth]{icml/img/optimal_dr_k2d_wm_compact.pdf}}
...
% *Here should be the description that the core idea is to learn a transformer model for next token prediction but enabler is actually data collection strategies.*

% *In the end of this section you should make a "podvodka" for the next paragraph. Something like "is this really neccessary or we can make it simpler" (do not use it like this, this is not academic, this is just the idea)*

% *Do not forget to include reference to Agentic Transformer and that it is actually a glitch in their evaluation setup but the idea is overall good*
...
% Algorithm Distillation (AD) \cite{laskin2022context} learns a map from trajectories to probabilities over actions by training a neural network $P$ with weights $\theta$ to minimize negative log likelihood loss: 

% $$
% \mathcal{L}(\theta):=-\sum_{t=1}^{T-1} \log P_\theta\left(a_t^{(n)} \mid h_{t-1}^{(n)}, o_t^{(n)}\right)
% $$
...
% This is a direct implication of the theoretical result discovered by \citet{jeon2024informationtheoretic}, which states that the misspecification error of Bayesian predictors trained on optimal and suboptimal data approaches zero, provided that the number of tasks and the sequence length tend toward infinity. To our knowledge, we are the first who experimentally prove this theoretic results.

\section{AD$^\varepsilon$}
\label{sec:ad_eps}
% To successfully learn a single policy that can achieve numerous tasks the data should contain policy improvements between transitions. Usually, multi-task meta-agents are trained on learning histories of single-task RL algorithms \cite{lee2023supervised, kirsch2022generalpurpose}. \citet{laskin2022context} showed that training meta-learners only on expert demonstrations is not enough for them to understand the task. However, this approach is accompanied by certain drawbacks. First of all, it is the substantial amount of data to collect that would be sufficient for training, since the data should encompass trajectories that achieve different goals. Secondly, RL algorithms that are used to collect data are notorious for their sample inefficiency and difficulty in converging. Is it possible to facilitate data acquisition by eliminating the requirement for RL algorithms to produce learning histories? What if we could utilize existing policies instead? 

% In our work we show that given a policy, it is possible to produce data suitable for In-Context RL. Moreover, we demonstrate the Transformer-like meta-learners are capable of outperforming suboptimal policies by learning on data synthetically generated with our method.
...
% \textbf{Linear Bandits.} We employ a linear bandit that is constructed similarly to \citet{lee2023supervised}. The reward function of the bandit is given by $r = \left\langle\theta_\tau, \phi(a)\right\rangle$, where $\theta_\tau \in \mathbb{R}^d$ is a context parameter that is different for each task, $\phi: \mathcal{A} \rightarrow \mathbb{R}^d$ is a projection from the action space to the embedding space. Note that we fix $\phi$ for all environments, while $\theta_\tau$ is task dependent.
...
% \begin{figure}[t!]
% % \begin{center}
%     \centerline{\includegraphics[width=0.9\columnwidth]{icml/img/opt_bandit.pdf}}
%     \caption{Results Linear bandits w/o noise; TODO}
%     \label{fig:optimal_bandits}
% % \end{center}
% \end{figure}
...
\begin{figure}[h]
    \centerline{\includegraphics[width=0.95\columnwidth]{icml/img/wm_screen.pdf}}
...
\vskip 0.2in
\begin{center}
    \centerline{\includegraphics[width=\textwidth]{icml/img/subopt_dr_k2d_wm_compact.pdf}}
...
% The main aim of our experiments is to determine whether it is possible to synthetically generate data to train meta-agents on. For our experiments, we employ AD meta-algorithm as described in \citet{laskin2022context}. We denote the AD model trained on synthetic data as AD$^\varepsilon$.


% First, we examine the capacity of synthetic data on a simple linear bandit. The bandit is constructed similarly to \citet{lee2023supervised}. The reward function of the bandit is given by $r = \left\langle\theta_\tau, \phi(a)\right\rangle$, where $\theta_\tau \in \mathbb{R}^d$ is a context parameter that is different for each task, $\phi: \mathcal{A} \rightarrow \mathbb{R}^d$ is a projection from the action space to the embedding space. Note that we fix $\phi$ for all environments, while $\theta_\tau$ is task dependent.  We take LinUCB \cite{NIPS2011_e1d5be1c} and Thompson Sampling \cite{thompson1933likelihood} as baselines. Theoretically, LinUCB's regret is bounded by $\widetilde{\mathcal{O}}(d \sqrt{K})$ for $d < |\mathcal{A}|$, $K$ is the number of steps. Thus, it should perform significantly better than Thompson Sampling. In contrast to LinUCB, Thompson Sampling does not utilize the linear structure of the problem. To obtain a data generating policy, we first train the posterior of Thompson Sampling and then employ the standard data collection procedure that is described in \autoref{subsec:gen}.

% \autoref{fig:optimal_bandits} shows that AD is able to learn on the generated data. It should be emphasized that the meta-learner's training extends beyond synthetic data; while the generating algorithm, Thompson Sampling, incurs higher asymptotic regret compared to LinUCB, AD$^\varepsilon$ is able to perform on par with LinUCB. 


% \begin{figure}[h!]
% % \begin{center}
%     \centerline{\includegraphics[width=0.8\columnwidth]{icml/img/huge_ts_eps.pdf}}
%     \caption{Huge TS for ADeps=0.5. Emphasize that we train ADeps on Noisy; TODO}
%     \label{fig:huge_ts}
% % \end{center}
% \end{figure}
...
% \paragraph{Can AD$^\varepsilon$ generate synthetical learning histories for in-context RL?}
...
% \textbf{If the generating policy for AD$^\varepsilon$ is suboptimal, is it possible for a meta-learner to outperform it?} 
...
% \textbf{Can AD$^\varepsilon$ be used in pixel-based environments?}
...
\vskip 0.2in
\begin{center}
    \centerline{\includegraphics[width=0.9\textwidth]{icml/img/anal_eps.pdf}}
...
% \paragraph{To what degree can a policy be suboptimal, yet still allow for an in-context agent to improve upon it?}
...
% \section{Learning from Optimal Policy}
% We first show that a meta-agent can indeed learn from synthetic learning histories when the generating policy is optimal.
% \subsection{Bandits}
% \subsection{POMDP}



% \section{Learning from Sub-Optimal Policy}
% \subsection{Bandits}
% \subsection{POMDP}
...
\begin{figure}[t!]
    \centerline{\includegraphics[width=0.8\columnwidth]{icml/img/anal_hist_2.pdf}}
...
% Meta-Reinforcement learning is a family of RL methods which aim is to learn generalized and sample efficient algorithms on top of inefficient and single-task common RL agents. A trained meta-RL model has learnt how to learn; in other words, it is able to explore an environment and can adapt to new goals. The classical approaches achieved this  by training a meta-agent through gradients of many on-policy RL algorithms, trained on different tasks \cite{finn2017model} or leveraging hidden states of RNN \cite{hochreiter2001learning, duan2016rl, wang2016learning}. More recent approaches use the in-context ability of Transformers by leveraging sequential improvements of RL algorithms during training  \cite{laskin2022context, lee2023supervised, sinii2023context, kirsch2023towards}. 

% On the other hand, meta-RL has its limitations; one of the most remarkable is the substantial data requirements. To efficiently meta-learn a set of tasks, the dataset needs to be large and diverse, as it should consist of different tasks for downstream RL algorithms to be trained on. Also, the data  must contain a sequential improvement from one transition to the other to enable an ability to explore. AD$^\varepsilon$ deals with these issues by being able to generate suitable synthetic data without explicitly training an RL agent to obtain learning histories. 

% \textbf{In-Context Learning.} 
% In-Context Learning is an emergent ability of LLM that enables a model to deduce the task and reason about it depending on the context it was provided with. A single sufficiently large pretrained LLM is able to solve a wide range of tasks, for example math problems \cite{wei2022chain}. Several studies have shown \cite{von2023transformers, dai2022can} that Transformers are able to perform gradient descent optimization in context, thus being able to learn from unseen data while their weights are frozen. In addition to that, \cite{muller2021transformers} showed that Transformers are able to perform Bayesian inference by approximating a posterior distribution given the data sampled from prior and put into context. These abilities made in-context learning suitable to transfer onto RL domain.

% \textbf{Offline Reinforcement Learning.}

% \begin{figure}[t]
% % \begin{center}
%     \centerline{\includegraphics[width=0.9\columnwidth]{icml/img/compare_bandit.pdf}}
%     \caption{Comparison of optimal and suboptimal ADeps; TODO}
%     \label{fig:compare_bandits}
% % \end{center}
% \end{figure}
...
% the possibilit which has been predicted theoretically in a recent study \cite{jeon2024informationtheoretic}
...
\begin{figure}[h!]
    \centerline{\includegraphics[width=0.8\columnwidth]{icml/img/rebut/optimal-ad-baseline-all.png}}
...
\begin{figure}[h!]
    \centerline{\includegraphics[width=0.4\columnwidth]{icml/img/rebut/subopt-k2d-and-bcformoracle.png}}
...
\begin{figure}[h!]
    \centerline{\includegraphics[width=0.6\columnwidth]{icml/img/rebut/ad1-0-starteps05.png}}
...
\begin{figure}[h!]
    \centerline{\includegraphics[width=0.5\columnwidth]{icml/img/anal_hist.pdf}}
