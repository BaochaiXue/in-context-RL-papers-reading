...
\begin{abstract}
% The overall message: the separation of mind and body, understanding the world (CWM), and learning the control (Agent) is more efficient for RL and in-context adaptation. Essentially, the separation means CMW never cares about the task reward, which is handled by the control agent.  The highlight is the two-fold in-context learning: in-context communication and in-context control. In-context communication, thanks to the model (GRU) and training (communication loss), enables CWM to understand the environment and how to share the understanding with the control agent. Incorporating the additional message, the control agent is able to adapt to new environments.  
...
% Reinforcement learning (RL) agents often struggle to generalize to new tasks and contexts without updating their parameters, mainly because their learned representations and policies are overfit to the specifics of their training environments. To boost agents' in-context RL (ICRL) ability, this work formulates ICRL as a two-agent emergent communication problem and introduces CORAL (Communicative Representation for Adaptive RL), a framework that learns a transferable communicative prior by decoupling representation learning from control. In CORAL, an Information Agent (IA) is pre-trained as a communicative dynamics model on a diverse distribution of tasks. Its objective is not to maximize task reward, but to build a robust world model and distill its understanding into concise messages. The content and form of these messages are shaped by a Causal Influence Loss, which uses a hybrid utility signal and KL Divergence to encourage the communication of decisive, high-utility information. During deployment, the previously trained IA serves as an expert ``prompter'' for a new Control Agent (CA), which learns to solve tasks by interpreting the provided communicative context. Our experiments demonstrate that this approach enables the CA to achieve significant gains in sample efficiency and successfully perform zero-shot adaptation to both new partners and entirely unseen sparse-reward environments, validating the efficacy of learning a transferable communicative representation.\footnote{Code and experimental details are available at \url{https://github.com/fernando-ml/CORAL}.}
\end{abstract}
...
%However, WM methods are plagued by sample inefficiency in training and objective mismatch between latent representation learning, aiming for a general dynamics model for various tasks, and task-specific policy learning \cite{}.
...
\vspace{-5pt}
    ...
    % \caption{\textbf{Overview of the CORAL framework for learning and deploying a communicative prior.} \textbf{(Left) Pre-training.} An Information Agent (IA, parameterized by $\phi$) and a Control Agent (CA, parameterized by $\theta$) are trained jointly on a distribution of tasks $\mathcal{T}$. At each timestep $t$, both agents receive a partial observation $o_t$. The IA generates a message $m_t$. The CA uses $o_t$ and $m_t$ to produce an action $a_t$. The CA is trained with a standard PPO objective $\mathcal{L}(\theta)$ based on the task reward $R_t$. The IA is trained with a loss $\mathcal{L}(\phi)$ composed of self-supervised objectives ($\mathcal{L}_{\text{WM}}, \mathcal{L}_{\text{Coh}}$) and a causal influence objective shaped by a utility signal $\mathcal{U}_t$ from the CA. \textbf{(Right) Deployment}. The pre-trained IA's weights are frozen. It is paired with a new, randomly initialized CA in an unseen task. The IA provides a continuous stream of messages, acting as a communicative prior to accelerate the CA's learning.}
...
%to new control agent partners and entirely unseen tasks. Our experiments show that when coupled with a new partner on an untrained task, our CORAL agent accelerates learning, reaching performance thresholds up to 3 times faster than standard PPO and over 5 times faster than an end-to-end world model.
...
% We extend this single-agent framework by incorporating a communicative partner that distills its experiences, based on its context, into a message $m_t$ that influences the acting agent's policy $\pi(\cdot|o_t,m_t)$. 

% This communicative agent needs to learn a communication protocol, a process often referred to as emergent communication~\cite{zhu2024survey}. A key challenge in this area is distinguishing meaningful communication from unfounded correlations. Effective communication requires both positive signaling (where the speaker agent produces messages correlated with its observation) and positive listening, where a listener agent's behavior is altered by those messages~\cite{lowe2019pitfalls}. As the latter is not guaranteed to arise from a standard RL objective, a central focus of our work is to design a framework that explicitly fosters and measures the causal impact of communication on an agent's policy.


% We formulate our problem as a Partially Observable Markov Decision Process (POMDP), where an agent learns a policy $\pi(a_t|o_t)$ to maximize the expected discounted return by mapping partial observations $o_t$ to actions $a_t$. Our CORAL framework structures this process through the interaction of two specialized modules: an Information Agent (IA) and a Control Agent (CA). At each timestep, both agents receive the same observation $o_t$. The IA, parameterized by $\phi$, produces a message $m_t \in\mathcal{M}$. The CA, parameterized by $\theta$, conditions its policy on both the observation and the message to select an action, $\pi_{\theta}(a_t|o_t, m_t)$.

% The CA is trained using Proximal Policy Optimization~\cite{schulman2017proximal}, an actor-critic algorithm that uses a learned value function $V_{\theta}(o_t, m_t)$ to estimate expected return. This value estimate is then used to compute the Generalized Advantage Estimate (GAE), $A_t$~\cite{schulman2015high}, which quantifies the relative value of a selected action. 

% This divergence is multiplied by a hybrid utility signal $\mathcal{U}_t$, which promotes that only beneficial communication is reinforced. The signal combines two sources of utility: The first is the Generalized Advantage Estimate (GAE)~\cite{schulman2015high}, $A_t$, which provides a low-variance estimate of the long-term, empirical advantage of taking an action. It is calculated as the exponentially-weighted average of temporal difference errors: $A_t = \sum_{k=0}^{\infty} (\gamma\lambda)^k \delta_{t+k}$, where $\delta_t = r_t + \gamma V_{t+1} - V_t$. The second source is the immediate change in the CA's own value estimate, $\Delta V_t = V_{\theta}(o_t, m_t) - V_{\theta}(o_t, \mathbf{0})$. After z-score normalization, these are combined and clipped to form the final positive utility signal:


% \section{Models}

% The basic model is defined as a nested tuple $(S, (\caln, O, (I,U)), (\calh, (M,V)), r, T)$
% \begin{itemize}
% \item 
...
% \begin{itemize}
% \item
% $\caln$: set of agents, $a\in \caln$,
% \item
% $O$: set of observation as the image domain of the observation function,
% \item
% $(I,U)$: set of actions for agent, $I$: inquiry action, $U$ sequential action, $u^a\in U$; 
% \end{itemize}
% \item
% Information explorers:
% \begin{itemize}
% \item
% $\calh$: set of information explorers, $b\in \calh$,
% \item
% $(M,V)$: set of actions for information explorers in the form of message-value pair, $(m_b^a, v_b^a)$;
% \end{itemize}
% \item
% $r$: reward;
% \item
...
% There seems no model for information exploration, the explorer seems to be assumed to know everything. 

% What is the form of information and message? Given a partial observation, there should be multiple states that can support the observation, and does information/message provide a probability distribution on these states?


% How to model dynamically changing environment? 
...
% We formalize the learning problem through the interaction of two specialized agents. The Information Agent (IA) is responsible for informed communication; its objective is to build a predictive model of the environment's dynamics and to distill this understanding into a useful communicative representation. Crucially, the IA's learning is not directly driven by the extrinsic task reward. In parallel, the CA is responsible for action and control leveraging the message communicated by the IA as a critical source of contextual information. 

% This establishes a clear functional decomposition. The IA's role is to learn a model of worlds, while the CA's role is to learn a control policy. The key to our framework lies in the learning objectives that couple these two processes through communication. We design the IA's objectives to be anticipatory, incentivizing it to produce messages that are maximally beneficial for the CA's decision-making process.
...
% We accomplish this by deriving the IA's learning signal directly from the CA's performance, thereby rewarding the IA for messages that induce beneficial changes in its partner's behavior.
...
% It is implemented as a recurrent neural network using a Gated Recurrent Unit (GRU) cell \cite{chung2014empirical}. The IA processes the current observation $o_t$ to update its internal hidden state $h_t^{\text{IA}}$, $h_t^{\text{IA}} = \text{GRU}_{\phi}(h_{t-1}^{\text{IA}}, o_t)$. This hidden state, which encodes a history of observations, then generates a message $m_t = f_{\phi}^{\text{msg}}(h_t^{\text{IA}})$.

% The Control Agent (CA), parameterized by $\theta$, is a feed-forward PPO network responsible for decision-making~\cite{schulman2017proximal}. It conditions its policy and value estimate on both its current observation $o_t$ and the message $m_t$ provided by the IA. The agent's policy $\pi_{\theta}(\cdot|o_t, m_t)$ and value function $V_{\theta}(o_t, m_t)$ are the outputs of a shared actor-critic network.
...
% \begin{equation*}
%     \mathcal{L}_{\text{Causal}}(\phi) = - \mathbb{E}_{\tau \sim \pi_{\theta, \phi}} \left[ \mathcal{U}_t \cdot D_{\text{KL}}\Big(\pi_{\theta}(\cdot|o_t, \mathbf{0}) \parallel \pi_{\theta}(\cdot|o_t, m_t)\Big) \right]
% \end{equation*}
...
% To this end, we deploy our generalist IA (pre-trained on a distribution of tasks but never on the specific evaluation environments) alongside a randomly initialized CA. This setup measures the pure in-context learning ability of the CA when guided by the IA's communicative protocol.

% The learning curves, presented in Figure~\ref{fig:learning-curves}, show that CORAL consistently outperforms both baselines across a range of unseen evaluation tasks. CORAL demonstrate better sample efficiency compared to PPO and the baseline world model, rapidly converging to high performance. This is quantitatively confirmed by our Time-to-Threshold (TTT) analysis in Table~\ref{table: TTT}, where it consistently reaches 90\% of the maximum performance in significantly fewer steps than the compared algorithms. For instance, in the largest available 


% The results, presented in Figure~\ref{fig:learning-curves} and Table~\ref{table: TTT}, show that the CORAL agent consistently and substantially outperforms both baselines across a range of unseen evaluation tasks. The learning curves in Figure~\ref{fig:learning-curves}...

% This performance gap is even more pronounced in more complex tasks (DoorKey, Crossings S11N5, Dynamic Obstacles). in 
...
% \begin{tabular}{lcccc}
% \toprule
% \textbf{Environment} & \textbf{Max. Perf.} & \textcolor[HTML]{923BE5}{CORAL} & \textcolor[HTML]{339332}{PPO} & \textcolor[HTML]{DD264A}{World Model} \\
% \midrule
% DoorKey 8x8 & 1.00 & 1.0 $\pm$ 0.2 & 2.3 $\pm$ 0.3 & 3.0 $\pm$ 0.4 (89\%) \\
% SimpleCrossing S11N5 & 1.00 & 1.3 $\pm$ 0.1 & 1.9 $\pm$ 0.3 & 6.9 $\pm$ 1.2 (19\%) \\
% DynObs Obstacles 6x6 - Random & 0.74 & 1.6 $\pm$ 0.2 & 1.5 $\pm$ 0.1 (97\%) & 1.3 $\pm$ 0.1 (97\%) \\
% LavaGap S7 & 1.00 & 0.3 $\pm$ 0.0 (95\%) & 0.4 $\pm$ 0.0 (95\%) & 0.4 $\pm$ 0.1 (94\%) \\
% Empty 16x16 & 1.00 & 0.5 $\pm$ 0.1 & 0.7 $\pm$ 0.1 & 0.5 $\pm$ 0.1 \\
% DynObs Obstacles 16x16 & 0.85 & 7.0 $\pm$ 0.8 (45\%) & 7.9 $\pm$ 0.5 (36\%) & 8.5 $\pm$ 0.7 (30\%) \\
% \bottomrule
% \end{tabular}
% \caption{Time-to-threshold analysis showing mean timesteps (in millions) ± 95\% confidence interval to reach 90\% of each environment's maximum performance. Percentages (\%) indicate success rate when less than 100\%. DNR indicates threshold never reached.}
% \label{table: TTT}
% \end{table*}
...
% the ability of our framework to generalize in a zero-shot setting where no further learning is permitted. In this experiment, we pre-trained the Information Agent on a simpler $\mathcal{T}$ (details in Appendix~\ref{appendix:hyperparameters_and_setup}), compared to the tested environments and in this case, we trained our control agent on a specific source task (\textit{DoorKey6x6}). The When then freeze the parameters of the CA and evaluate their performance directly on a more complex, unseen task (e.g., \textit{DoorKey8x8}). 
...
% Trained message with untrained CA vs Random message with untrained CA vs No Message with untrained CA.

% To verify that communication is causally responsible for the observed performance gains, we analyze the protocol's impact on the Control Agent's policy. We introduce a diagnostic metric, the Instantaneous Causal Effect (ICE), inspired by the Causal Influence of Communication~\cite{lowe2019pitfalls}. The ICE quantifies the immediate, per-step influence of a message by measuring the Reverse KL Divergence between the CA's policy with and without the communication:

% \begin{align*}
%     \text{ICE}_t =D_{\text{KL}}\Big(\pi_{\theta}(\cdot|o_t, \mathbf{0}) \parallel \pi_{\theta}(\cdot|o_t, m_t)\Big)
%     \label{eq:ice}
% \end{align*}
...
% Hello~\ref{appendix:code_availability}



% \begin{enumerate}
%     \item Results Training (Preferably Appendix). Training plots for all the environments during the Deployment Stage (Trained Controller/Untrained Controller)
%     \begin{itemize}
%         \item 
%     \end{itemize}
%     \item Pre-Trained CWM + Trained Controller in previously seen envs v.s. Single Agent Baseline
%     \item Pre-Trained CWM + Untrained Controller in unseen different class env. Executed
%     \item Pre-Trained WM (exposed to all) + Trained Controller (limited to one env) deployed in different env. In the Make. Envs for main: Doorkey8x8, SimpleCrossingS11N5, DynamicObs, LavaGapS7.
%     \item Ablation: Communication with Random Message vs. Ours
%     \item CIC \cite{lowe2019pitfalls}. Implemented \& collecting for new runs!
% \end{enumerate}
...
% \paragraph{Scalability} The communication protocol in CORAL is based on a fixed-dimensional dense vector, which, while effective in the studied domains, may not scale to tasks requiring more 
...
% \paragraph{Extended Validation} tackling more sophisticated tasks, such as Atari games, Mujuco control, and Minecraft, check Jimmy Ba's paper.

% \paragraph{Multi-Agent Communication} Our work right now considers two agents; in the future, we can consider an information agent that coordinates with multiple specialized control agents.
