...
% The potential development of in-context reinforcement learning is currently limited by all the factors outlined above. We believe it is necessary to address these barriers, given it's essential role on the path to foundation and truly generalist agents \citep{team2021open, team2023human, kirsch2023towards, lu2024structured, liu2024position}. 

% The development of in-context reinforcement learning is currently hindered by these factors. We believe it is crucial to address these barriers, given the essential role of in-context learning in the path to foundation and truly generalist agents \citep{team2021open, team2023human, kirsch2023towards, lu2024structured, liu2024position}. 
...
% \rowcolor{lightblue!80} XLand-100B (ours)   & 30,000   & 100 B          & 200 M       & 320GB   & \cmark      & \cmark      \\
...
% [tbd] paragraph about: in-context rl history + link to the meta-rl
% Despite the rapid adoption of the transformer architecture in reinforcement learning \citep{hu2022transforming, agarwal2023transformers, li2023survey} after the release of Decision Transformer (DT) \citep{chen2021decision, janner2021offline}, models with in-context learning capabilities appeared much later, for a number of reasons.  First, to transition from in-weights to in-context learning, the model should be trained on tens of thousands of unique tasks \citep{kirsch2022general}. Unfortunately, as we explain in more detail in \Cref{missing-data}, even the largest reinforcement learning datasets currently have a number of tasks in the order of hundreds \citep{padalkar2023open} or smaller. Second, it was necessary to find the right context format and data collection pipeline, which for many methods was different from what is commonly available in existing datasets. 
...
% After pretraining, which requires the access to the full learning histories of base single-task algorithms, AD is capable of generalizing to new unseen tasks, solving them with greater sample efficiency that the data-generating policy. 
...
% After pre-training, DPT is capable of efficient exploration and generalization to new tasks. As \citet{lee2023supervised} proves, DPT actually performs posterior sampling \citep{osband2013more} to achieve this.
...
\vskip -0.2in
    ...
    % Agent observes a small area around it, can open doors or pick up and put down objects to trigger the rules or complete goals. 
...
% Starting from the seminal work of \citet{wang2016learning, duan2016rl, finn2017model} on meta-RL, much of the subsequent work \citep{zintgraf2019varibad, melo2022transformers, grigsby2023amago, lu2024structured, shala2024hierarchical, beck2024recurrent} has focused on environments that either have very simple task distributions, such as various bandits, simple navigation to specified unknown goals, few-shot control adaptation for varying characteristics such as velocity in MuJoCo \citep{todorov2012mujoco}, or have very small and limited distributions of hard tasks such as in the Meta-World benchmark \citep{yu2020meta}. This is because, to generalize in meta-RL, training needs to be performed on many different tasks, significantly increasing the cost and time required for experimentation. Recently, \citet{nikulin2023xland} released XLand-MiniGrid, a GPU-accelerated environment and million-task benchmarks that significantly lowered the entry barrier for meta-RL research. We will describe it shortly here.

% The recent wave of research on in-context reinforcement learning \citep{laskin2022context, lee2023supervised, norman2023first, kirsch2023towards, sinii2023context, zisman2023emergence} has inherited the benchmarks from the meta-RL literature and thus also compares new methods on quite simple tasks. There are a few notable exceptions, such as XLand \citep{team2021open, team2023human} and Alchemy \citep{wang2021alchemy}, which cover large and diverse distributions of tasks. However, the XLand environment is not open source, while Alchemy only runs at 30 FPS, which is not scalable. The reason for this choice of benchmarks is that meta-RL, as well as in-context RL, requires much more resources than conventional RL, since in order to generalize, training or data collection must be performed on many different tasks, which significantly increases the cost and time of experimentation. Recently, \citet{nikulin2023xland} released XLand-MiniGrid, a GPU-accelerated environment as well as benchmarks that significantly lowered the entry barrier for meta-RL research. We will describe it shortly here.
...
% \section{There are no other datasets for In-Context RL}
% \section{First Dataset for In-Context RL}
...
% The code is open-source and available under Apache 2.0 licence at \url{https://github.com/dunno-lab/xland-minigrid-datasets}. 
...
% \vskip -0.2in
%     \centering
%     \includegraphics[width=0.4\textwidth]{images/pretrain-return.pdf}
%     \caption{Evaluation return for multi-task goal-conditioned reccurent PPO pretraining on 65k tasks. Pretrained agent was further used a starting point for single-task finetuning during dataset collection.}
...
% \vskip -0.2in
%     \centering    
%     \includegraphics[width=0.4\textwidth]{images/pre-vs-scratch-return-hard_v2.pdf}
%     \caption{Single-task learning curves on 36 hard tasks for policies trained from scratch or fine-tuned from multi-task pre-trained checkpoints. See \Cref{app:finetune} for curves on tasks of all difficulty.}
%     \label{fig:pretrain-vs-finetune}
% \vskip -0.2in
% \end{wrapfigure}

\textbf{Base algorithm.}
% For the collection of complete learning histories, a base single-task RL algorithm is required. It must satisfy three criteria. First, it should work in POMDP \citep{aastrom1965optimal}, since XLand-MiniGrid is a partially observable environment with high episodic memory requirements. Second, it should be as efficient as possible in terms of wall clock time to cope with the collection of tens of thousands of tasks. Third, is should be sample-efficient, otherwise, each learning history would be too long, inflating the dataset size. 
...
% \vskip -0.2in
%     \centering
%     \includegraphics[width=0.4\textwidth]{images/num-filtered-rules.pdf}
%     \caption{Distribution of the tasks by difficulty sampled initially and in the resulting dataset. To ensure the quality, we filtered tasks where the final return was below $0.3$ or the data was corrupted due to some errors during training.}
%     \label{fig:filter-vs-actual}
% \vskip -0.2in
% \end{wrapfigure}
...
% \vskip -0.2in
%     \centering
%     \includegraphics[width=0.4\textwidth]{images/learning-history-per-num-rules_v2.pdf}
%     \caption{Learning histories for the XLand-100B dataset separated by number of rules. For visual clarity, we show only a sample of the possible number of rules and normalize the number of episodes, as they vary considerably between difficulties.}
%     \label{fig:hist-per-rules}
% \vskip -0.2in
% \end{wrapfigure}
...
% \citet{lee2023supervised} showed that DPT can still achieve good performance when trained on the actions of the trained expert, which may not be globally optimal, e.g. in our case such expert is the final PPO policy after fine-tuning. 
...
% % \vskip 0.2in
% \begin{center}
% \centerline{\includegraphics[width=\textwidth]{images/actions-agreement.pdf}}
%     \caption{TODO}
%     \label{fig:act-agg}
% \end{center}
% % \vskip -0.2in
...
% % \vskip 0.2in
% \begin{center}
% \centerline{\includegraphics[width=\textwidth]{images/learning-history.pdf}}
%     \caption{TODO}
%     \label{fig:act-agg}
% \end{center}
% % \vskip -0.2in
...
% % \vskip 0.2in
% \begin{center}
% \centerline{\includegraphics[width=0.5\textwidth]{images/return-per-rules.pdf}}
%     \caption{TODO}
%     \label{fig:act-agg}
% \end{center}
% % \vskip -0.2in
...
% % \vskip 0.2in
% \begin{center}
% \centerline{\includegraphics[width=0.5\textwidth]{images/pre-vs-scratch-bar.pdf}}
%     \caption{TODO}
%     \label{fig:act-agg}
% \end{center}
% % \vskip -0.2in
...
% % \vskip 0.2in
% \begin{center}
% \centerline{\includegraphics[width=0.5\textwidth]{images/pre-vs-scratch-return-hard.pdf}}
%     \caption{TODO}
%     \label{fig:act-agg}
% \end{center}
% % \vskip -0.2in
...
% % \vskip 0.2in
% \begin{center}
% \centerline{\includegraphics[width=\textwidth]{images/learning-history-per-num-rules.pdf}}
%     \caption{TODO}
%     \label{fig:act-agg}
% \end{center}
% % \vskip -0.2in
...
% % \vskip 0.2in
% \begin{center}
% \centerline{\includegraphics[width=0.5\textwidth]{images/episodes-per-rules.pdf}}
%     \caption{TODO}
%     \label{fig:act-agg}
% \end{center}
% % \vskip -0.2in
...
% % \vskip 0.2in
% \begin{center}
% \centerline{0.5\includegraphics[width=\textwidth]{images/num-filtered-rules.pdf}}
%     \caption{TODO}
%     \label{fig:act-agg}
% \end{center}
% % \vskip -0.2in
...
% \begin{figure}[h!]
% \begin{center}
% \centerline{\includegraphics[width=\textwidth]{images/seq_len_perf_T.pdf}}
%     \caption{AD performance on our datasets for different sequence lengths. Both datasets lead to the emergence of in-context ability in transformers. We report the average return on $1024$ unseen tasks across $3$ seeds with the shaded region of $1$ std.}
%     \label{fig:ad-agg}
% \end{center}
% % \vskip -0.2in
% \end{figure}
...
\begin{figure}[t]
\begin{center}
\centerline{\includegraphics[width=\textwidth]{images/data-vs-num_goals.pdf}}
...
% We report the mean on $1024$ train tasks for the AD, same tasks are taken from the dataset to report. AD's performance is averaged across $3$ seeds, shaded region is $1$ std.
...
% % \vskip 0.2in
% \begin{center}
% \centerline{\includegraphics[width=0.8\textwidth]{images/dpt/dpt_trivial_test_multi_seq.pdf}}
%     \caption{TODO}
%     \label{fig:dpt-seqlen-trivial-test}
% \end{center}
% % \vskip -0.2in
% \end{figure}
...
% TODO: camera ready to cite "some" offline rl papers
% \section{Beyond In-Context RL}
...
% % TODO: move to the appendix and/or refomulate for the introduction
% \textbf{In-Context Learning.} In-context learning, i.e. the ability to learn new tasks purely based on examples given in the context during inference and without any weight updates, was first manifested itself in large language models, such as GPT-3 \citep{brown2020language}. Further, with a gradual increase of the context size supported in the recent foundation models from hundreds to millions \citep{reid2024gemini, liu2024world, ding2024longrope} it became recognised that in many tasks in-context learning has a performance on par or better than task specific fine-tuning \citep{bertsch2024context, agarwal2024many}, as with such large contexts entire datasets can be used as a prompts.

% Initially, it was thought that in-context learning abilities only emerged in large transformers \citep{vaswani2017attention}. Recently, however, it has been discovered that small transformers are also capable of in-context learning \citep{kirsch2022general, von2023transformers}, and even many non-transformer models such as novel state-space sequence models \citep{gu2023mamba}, RNNs or even MLPs have such abilities \citep{bhattamishra2023understanding, akyurek2024context, park2024can, grazzi2024mamba, vladymyrov2024linear, tong2024mlps}, although somewhat weaker than in transformers with exact attention. In-context learning is also not specific to language modeling and has appeared in other domains, e.g. image generation \citep{bai2023sequential, najdenkoska2023context, doveh2024towards, tian2024visual}. It is therefore suggested that in-context capabilities are not a property of specific model architectures, but rather a specific emergent property of certain training data distributions \citep{chan2022data, gu2023pre}.

\clearpage
% \section{In-Context Algorithms Description}
% \subsection{AD}
...
% While training, AD uses transformer backbone and predicts the next action given the history of interactions with the environment. In case of discrete action space, the negative log-likelihood loss is used. 
...
% DPT uses the same scheme of training and optimization as AD.
...
% We also show training logs for both datasets. Trivial: \href{}{\color{blue}{wandb [available after de-anon]}}; medium: \href{}{\color{blue}{wandb [available after de-anon]}}
...
% We also show DPT training logs. Trivial: \href{}{\color{blue}{wandb [available after de-anon]}}; medium: \href{}{\color{blue}{wandb [available after de-anon]}}
...
% We also show logs for Key-To-Door Experiments: Markovian: \href{}{\color{blue}{wandb [available after de-anon]}}; non-Markovian: \href{}{\color{blue}{wandb [available after de-anon]}}
...
% \begin{table}[h]
% \centering
% \caption{DPT Hyperparameters on Key-To-Door}
% \label{table:dpt-k2d-hypers}
% \begin{tabular}{@{}lll@{}}
% \toprule
% Hyperparameter                  & Value \\ 
% \midrule
...
% \vskip 0.2in
\begin{center}
\centerline{\includegraphics[width=\textwidth]{images/dpt/dpt-k2d.pdf}}
...
% \begin{table}[h!]
% \centering
% \caption{Last episode returns for for AD algorithm on our datasets.}
% \label{tab:results}
% \begin{tabular}{@{}lllll@{}}
% \toprule
% Dataset              & \multicolumn{4}{c}{Seq. length} \\ \midrule
%  & \multicolumn{1}{c}{\textbf{512}} & \multicolumn{1}{c}{\textbf{1024}} & \multicolumn{1}{c}{\textbf{2048}} & \multicolumn{1}{c}{\textbf{4096}} \\ \cmidrule(l){2-5} 
% \textbf{trivial-20B} & 0.403  & 0.306  & 0.294 & 0.234 \\
% \textbf{medium-100B} & 0.416  & 0.400  & 0.324 & 0.200 \\ \bottomrule
% \end{tabular}
% \end{table}
...
% \vskip 0.2in
\begin{center}
\centerline{\includegraphics[width=0.8\textwidth]{images/model-vs-data-bar.pdf}}
...
% \vskip 0.2in
\begin{center}
\centerline{\includegraphics[width=\textwidth]{images/data-vs-num_goals_full.pdf}}
...
% \vskip 0.2in
\begin{center}
\centerline{\includegraphics[width=0.8\textwidth]{images/dpt/dpt_seq_len_perf.pdf}}
...
% \vskip 0.2in
\begin{center}
\centerline{\includegraphics[width=0.8\textwidth]{images/dpt/dpt-model-vs-data-bar.pdf}}
...
% \vskip 0.2in
\begin{center}
\centerline{\includegraphics[width=\textwidth]{images/dpt/dpt-data-vs-num_goals.pdf}}
...
\begin{minipage}[t]{.31\textwidth}
  ...
  % \captionof{figure}{Vizualization of rulesets with increasing difficulty from medium-1m benchmark.}
...
% \vskip 0.2in
\begin{center}
\centerline{\includegraphics[width=0.5\textwidth]{images/exp_data_goals.png}}
