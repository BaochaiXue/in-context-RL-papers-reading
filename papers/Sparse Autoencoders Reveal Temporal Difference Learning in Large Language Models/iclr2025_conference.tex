...
% TODO
%Deactivating and adversarially clamping the TD representations weaken the LLMs' RL abilities, and change their representations in subsequent layers.

% Using this methodology we uncover the components of TD learning in Llama, showing \emph{how} an LLM implements a domain-specific algorithm in-context.  


% show that in-context learning in LLMs can be studied and manipulated systematically using SAEs. With our methodology we extract representations that closely match . establishing not only \emph{that} LLMs can perform RL in-context 

% In sum, our findings suggest LLMs can implement 
% that large-scale self-supervised training on text can give rise to domain-specific algorithms like TD learning. 
...
%     \text{TD}~{\text{error}} = \reward + \gamma \max_a \Qtarget - \Q
% \end{equation}
\begin{align}
...
% \includegraphics[width=0.5\textwidth]{figures/TD_qualitative.pdf}
% \caption{Qualitative comparison of the TD error and the best matching SAE feature from block 34 for three separate runs in all three variations of the Two-Step Task. The SAE feature shows similar jumps as TD when Llama encounters surprising events, such as transitioning to unexpected states or receiving an unexpected reward. The dashed line indicates the onset of the change in either transition dynamics or reward function.}\label{fig:TD}
% \end{wrapfigure}
...
% Taken together, these findings provide strong evidence that Llama's graph knowledge is represented as SR and is acquired through TD learning. 
...
% Recent studies have shown that LLMs can learn predictive representations not only of the upcoming token, but also of future tokens \citep{wu2024language}, with standard next-token prediction objectives. Training LLMs to predict not only the next token but multiple tokens into the future, has also been shown to improve next-token prediction accuracy \citep{gloeckle2024better}. 


% Lastly, TD learning has played an important role in cognitive neuroscience. Several studies have proposed a link between dopamine, the striatum and TD errors \citep{schultz_neural_1997, daw_model-based_2011, fiorillo2003discrete}. Our work is one of several to suggest lines of similarity between LLM representations and representations supporting human cognitive functions like language \citep{schrimpf2021neural, tuckute2024driving}, vision \citep{schrimpf2018brain} and generalization \citep{demircan_language_2023}.
...
% \begin{equation}\label{eq:count_transition}
% \mathbf{T}(s, s') \leftarrow \frac{\mathbf{N}(s, s') + \mathbb{I}(s_t = s, s_{t+1} = s')}{\sum_{s''} [\mathbf{N}(s, s'') + \mathbb{I}(s_t = s, s_{t+1} = s'')]}
% \end{equation}

% where $\mathbf{N}$ is the unnormalised transition matrix estimate. 
...
% \begin{equation}\label{eq:surprise_equation}
% \text{surprise}(s, s') = [\mathbb{I}(s_t = s, s_{t+1} = s') -\log \mathbf{T}(s, s')] + [(1 - \mathbb{I}(s_t = s, s_{t+1} = s')) -\log (1 - \mathbf{T}(s, s'))]
% \end{equation}
